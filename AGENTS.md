...
External tools are shipped as plugins using `@cireilclaw/sdk`. Plugins run in isolated worker threads and are loaded from `config/plugins.toml`. Built-in examples live in `packages/brave-search/` and `packages/openweather/`. The template at `packages/template/` can be used as a starting point.
...
4. Processes tool outputs (built-in tools: file I/O, exec, respond, react, session queries, scheduling, etc.; plus any plugin tools)
5. Persists session to SQLite

### Global Config (`~/.cireilclaw/config/`)

```
engine.toml                 # Global default providers
system.toml                 # Optional system-wide settings
plugins/                    # Plugin-specific configs (e.g. brave-search.toml)
plugins.toml                # Plugin registry
migrations.json             # Applied config migration state
backups/                    # Config migration backups
```
...
config/          # engine.toml, tools.toml, heartbeat.toml, cron.toml, conditions.toml, sandbox.toml
  channels/      # Channel configs (discord.toml)
...
- TypeScript strict mode with `noUncheckedIndexedAccess`, `noImplicitOverride`, `verbatimModuleSyntax`, `noUnusedLocals`, and `noUnusedParameters`.
