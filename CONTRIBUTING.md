...
There is a [proposal template](.github/ISSUE_TEMPLATE/proposal-for-a-feature.md) for feature discussions.
...
- This is a pnpm workspace monorepo. The main application lives in `packages/runtime/`; additional packages (e.g. plugins, SDK) live under `packages/`.
- No build step. TypeScript runs directly via `tsx`.
- Path alias: within `packages/runtime`, `#` maps to `./src/` (e.g., `import { foo } from "#engine/index.js"`). **Always use the `.js` extension in imports** even for `.ts` files (due to NodeNext requirement).
...
- TypeScript strict mode is enforced, including `noUncheckedIndexedAccess`, `noImplicitOverride`, `verbatimModuleSyntax`, `noUnusedLocals`, and `noUnusedParameters`.
...
Used for `sessions.db`. If you modify `packages/runtime/src/db/schema.ts`:

1. Generate a migration: `pnpm --filter cireilclaw-runtime exec drizzle-kit generate`
2. Verify the generated SQL in `packages/runtime/drizzle/`.

### Config Migrations

Used for TOML files (e.g., `engine.toml`, `tools.toml`). If you change a config schema in `packages/runtime/src/config/schemas/`:

1. Add a new migration in `packages/runtime/src/config/migrations/YYYYMMDDHHMMSS_name/migration.ts`.
...
- Copy `packages/runtime/src/config/migrations/template.ts` as a starting point.

2. Test it using `pnpm start migrate --dry-run`.

Migrations are auto-discovered from the directory at runtime; no manual registration is required.

## Adding New Tools

Built-in tools live in `packages/runtime/src/engine/tools/`. Each built-in tool requires:

1. A `ToolDef` implementation.
2. A Valibot schema for input validation.
3. Registration in `packages/runtime/src/engine/tools/index.ts`.
...
Tools can also be shipped as external plugins using `@cireilclaw/sdk`. Plugin packages (see `packages/brave-search/` and `packages/openweather/` for examples) run in isolated worker threads and are configured via `config/plugins.toml`. The template at `packages/template/` can be used as a starting point for new plugins.
...
- `packages/runtime/src/util/paths.ts` (Path allowlists and sandbox mapping)
- `packages/runtime/src/util/sandbox.ts` (Bubblewrap configuration)
- `packages/runtime/src/engine/tools/exec.ts` (Command execution logic)
